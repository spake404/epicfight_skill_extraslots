...
public final class ExtraSlotsRuntimeExpander {
	...
	private static Method categoryRemoveMethod;

	private ExtraSlotsRuntimeExpander() {
	}

	public static void expand(PlayerPatch<?> playerPatch) {
		expand(playerPatch, ExtraSlotCounts.configured());
	}

	public static void expand(PlayerPatch<?> playerPatch, ExtraSlotCounts counts) {
		if (playerPatch == null) {
			return;
		}

		expand(playerPatch.getSkillCapability(), playerPatch, counts);
	}

	public static void expandAndClean(PlayerPatch<?> playerPatch) {
		expandAndClean(playerPatch, ExtraSlotCounts.configured(), null);
	}

	public static void expandAndClean(PlayerPatch<?> playerPatch, ExtraSlotCounts counts) {
		expandAndClean(playerPatch, counts, null);
	}

	public static void expandAndClean(PlayerPatch<?> playerPatch, ExtraSlotCounts counts, ServerPlayer player) {
		if (playerPatch == null) {
			return;
		}

		expand(playerPatch, counts);
		clearDisabledSlots(playerPatch.getSkillCapability(), counts, player);
	}
	...
	public static void clearDisabledSlots(CapabilitySkill skills, ExtraSlotCounts counts) {
		clearDisabledSlots(skills, counts, null);
	}

	public static void clearDisabledSlots(CapabilitySkill skills, ExtraSlotCounts counts, ServerPlayer player) {
		if (skills == null) {
			return;
		}
	...
	public static void expand(CapabilitySkill skills, PlayerPatch<?> playerPatch) {
		expand(skills, playerPatch, ExtraSlotCounts.configured());
	}

	public static void expand(CapabilitySkill skills, PlayerPatch<?> playerPatch, ExtraSlotCounts counts) {
		...
		try {
			...
			boolean expandedContainers = false;

			if (current == null) {
				current = new SkillContainer[0];
			}

			SkillContainer[] expanded = current.length >= requiredLength ? current : Arrays.copyOf(current, requiredLength);
			...
			for (SkillSlot slot : slots) {
				...
				if (ordinal < 0 || ordinal >= expanded.length) {
					continue;
				}
				...
				if (expanded[ordinal] == null) {
					...
					expandedContainers = true;

					if (!ExtraSkillSlots.isManagedSlot(slot)) {
						categoryPutMethod.invoke(containersByCategory, slot.category(), container);
					}
				}
			}

			if (expandedContainers || expanded != current) {
				skillContainersField.set(skills, expanded);
			}

			syncCategoryIndex(containersByCategory, expanded, slots, counts);
	...
	private static void ensureReflection() throws NoSuchFieldException, NoSuchMethodException, ClassNotFoundException {
		...
		categoryRemoveMethod = Class.forName("com.google.common.collect.HashMultimap").getMethod("remove", Object.class, Object.class);
	}

	private static void syncCategoryIndex(Object containersByCategory, SkillContainer[] containers, Collection<SkillSlot> slots, ExtraSlotCounts counts) throws ReflectiveOperationException {
		if (containersByCategory == null) {
			return;
		}
		...
		for (SkillSlot slot : slots) {
			...
			if (ordinal < 0 || ordinal >= containers.length) {
				continue;
			}

			SkillContainer container = containers[ordinal];

			if (container == null || !ExtraSkillSlots.isManagedSlot(slot)) {
				continue;
			}

			if (ExtraSkillSlots.isEnabled(slot, counts)) {
				categoryPutMethod.invoke(containersByCategory, slot.category(), container);
			} else {
				categoryRemoveMethod.invoke(containersByCategory, slot.category(), container);
			}
		}
	}
